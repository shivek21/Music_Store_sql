...
SELECT billing_city, sum(total) AS net_invoice FROM invoice
GROUP BY billing_city
ORDER BY net_invoice DESC;

-- Q5- Who is the best customer? The customer who has spent the most money will be
-- declared the best customer. Write a query that returns the person who has spent the
-- most money
SELECT customer.customer_id,customer.first_name,customer.last_name,sum(invoice.total) as net_invoice
FROM customer
...
(
	SELECT customer.customer_id,customer.first_name,customer.last_name,invoice.billing_country,SUM(invoice.total) AS total_spends,
    ROW_NUMBER() OVER(PARTITION BY invoice.billing_country ORDER BY SUM(invoice.total)) AS ROW_NO
    FROM invoice
    JOIN customer ON customer.customer_id=invoice.customer_id
    GROUP BY customer.customer_id,customer.first_name,customer.last_name,invoice.billing_country
    ORDER BY billing_country ASC
)
SELECT * FROM customer_max_spent
WHERE ROW_NO<=1;
